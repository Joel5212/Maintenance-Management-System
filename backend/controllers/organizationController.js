...
const registerOrganization = async (req, res) => {
    ...
    try {
        const { email, password, name, phoneNumber, orgName, orgCity, orgState, orgZipCode, orgCountry, orgPhoneNumer } = req.body;

        const salt = await bcrypt.genSalt(10)
        const hash = await bcrypt.hash(password, salt)

        const organization = { orgName, orgCity, orgState, orgZipCode, orgCountry, orgPhoneNumer }
